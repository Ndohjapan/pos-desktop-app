  ...
  return (
    <div className="w-full px-6 md:px-10 pt-4">
      <div className="flex items-center gap-2 overflow-x-auto pb-1">
        ...
        {parked.map((order) => (
          // Whole chip is the resume target (easier to tap than a small button).
          <div
            key={order.id}
            onClick={() => resume(order)}
            role="button"
            title="Tap to resume this order"
            className="flex items-center gap-2 bg-warning-50 border border-warning-100 rounded-full pl-2 pr-1 py-1 whitespace-nowrap cursor-pointer hover:bg-warning-100 active:scale-[0.98] transition-all"
          >
            <span className="flex items-center justify-center w-7 h-7 rounded-full bg-primary-700 text-white shrink-0">
